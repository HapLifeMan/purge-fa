...
let purge = function(scanContentInFolder, fontAwesomeSrcFolder, fontAwesomeDistFolder, whitelist) {
    ...
    for(let index in whitelist) {
        ...
        // If there is more than 1 icon in this whitelist type
        if(whitelist[index] instanceof Array) {

            // For each icon in array
            for(let item in whitelist[index]) {

                // Add it to corresponding bigData
                bigData[index].push(whitelist[index][item])

            }

        }

        // Else icon is alone in this whitelist type
        else {
